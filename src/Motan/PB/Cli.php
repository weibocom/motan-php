...
class Cli
{
    ...
    public static function runProtoc($pluginExecutable)
    {
        ...
        if ($result->options['weibo']) {
            $args['weibo'] = 1;
        }
        if ($result->options['weibojson']) {
            $args['weibojson'] = 1;
        }
    ...
    public static function parseArguments()
    {
        ...
        $main->addOption('weibo', array(
            'short_name' => '-W',
            'long_name' => '--weibo',
            'action' => 'StoreTrue',
            'description' => 'turn on weibojson file generation',
        ));

        $main->addOption('weibojson', array(
            'short_name' => '-w',
            'long_name' => '--weibojson',
            'action' => 'StoreTrue',
            'description' => 'turn on weibojson file generation',
